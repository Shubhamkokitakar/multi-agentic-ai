...
cols_to_index = ['SupplierName', "Taxonomy", "BusinessUnits","UnitOfMeasure",'CSID']
...
dbutils.widgets.multiselect('indexName', 'All', ['All'] + cols_to_index, 'Select Indexes to Update')
...
combined_spend = dls.spark_session.read.parquet(f"{adls_path}/PSCM/SSV/CombinedSpend")
...
def upload_data_to_cog_search(data, column, start_index = 0, window = 10000, credential=credential, endpoint = endpoint):
    
    index_name = getIndexName(column)
    search_client = SearchClient(endpoint = endpoint, index_name = index_name, credential=credential)

    if column == "Taxonomy":
        columns=['ChevronTaxonomyLevel1', 'ChevronTaxonomyLevel2', 'ChevronTaxonomyLevel3', 'ChevronTaxonomyLevel4', 'BUCategory', 'EnterpriseCategory','BUSubCategory' ]
    elif column == "BusinessUnits":
        columns=['BUMDMRegion', 'BUMDMCountry', 'Address', 'PlantName', 'BusinessUnitID', 'L4BU', 'L5BU']
    elif column == "CSID":
        columns=['CSIDSupplier', 'CSIDParent']
    else:columns=[column]

    for column_ in columns:
        data_ = data.withColumn('column_name', lit(column_)).select(
            col(column_).alias('value'), 
            col(f"{column_}GUID").alias('id'), 
            "column_name"
        ).dropDuplicates(['value'])

        df = data_.filter((col('value') != "None") & (col('value').isNotNull()) & (col('value') != "null")).na.drop(subset=['value'])
        total_size = df.count()
        ...
        for start in range(0, total_size, window):
            print(f"Processing records from {start_index} to {start_index + window - 1}", end='\r')
            filtered_df = df.filter( (col("index") >= start_index) & (col("index") < (start_index+window))).drop("index")
...
for index_col in indexes_to_update:
    ...
    upload_data_to_cog_search(combined_spend, index_col, window = 32000)
